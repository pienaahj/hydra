...
func runUDPClient(address string) error {
	...
	for scanner.Scan() && err == nil {
		...
		fmt.Println("What message would yopu like to send?")
	}
	return nil
}
...
/*
 to run:
 server -  go run udpcommunications.go -type s -addr :8787
 client - go run udpcommunications.go -type c -addr :8787
*/
